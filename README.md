...
>
> v1.0.8：单文件直开（拖拽或选择一个视频文件：直接定位播放该文件、立即开播，文件列表浮层本次会话默认收起——列表仍按其所在目录构建，连播 / 上一个下一个 / T 键打开列表一切照常）
>
...
- **拖拽即开**：把文件夹拖到窗口任意位置即可开始观看；拖拽**单个视频文件**则直接定位播放该文件（v1.0.8，列表浮层本次会话默认收起，T 键随时打开）
- **点选即开**：引导页大号「+」按钮弹出系统目录选择对话框，与拖拽完全等效（无需鼠标拖拽也能用）；「打开单个视频文件」次级入口直接播放选中的文件
